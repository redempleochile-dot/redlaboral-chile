import uuid, json, random, io, base64, qrcode, time, csv, threading
...
from .forms import (
    ...
    ValoracionForm, SuscriptorForm, PerfilEmpresaForm, ReporteForm, PreguntaForm
)

def pagina_exito(request): return render(request, 'exito.html')
...
def detalle_oferta(request, id):
    ...
    oferta.visitas += 1; oferta.save(update_fields=['visitas'])
    ...
    if request.method == 'POST' and 'btn_preguntar' in request.POST:
        ...
        if form_pregunta.is_valid():
            preg = form_pregunta.save(commit=False); preg.oferta = oferta; preg.usuario = request.user; preg.save()
            if oferta.usuario: Notificacion.objects.create(usuario=oferta.usuario, mensaje=f"❓ Nueva pregunta en {oferta.titulo}", enlace=f"/oferta/{oferta.id}/")
            messages.success(request, "Pregunta enviada."); return redirect('detalle', id=id)
    else: form_pregunta = PreguntaForm()

    match_percent = None; match_details = []
    if request.user.is_authenticated:
        try:
            candidato = request.user.candidato; score = 0
...
def publicar_empleo(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.user.is_authenticated: oferta.usuario = request.user
            oferta.save()
            ...
            if oferta.email_contacto:
                try: link = request.build_absolute_uri(f"/oferta/editar/{oferta.token}/"); send_mail(f"Gestiona: {oferta.titulo}", f"Link secreto:\n{link}", settings.EMAIL_HOST_USER, [oferta.email_contacto], fail_silently=True)
                except: pass
            return redirect('pagina_exito') 
    else: form = NuevaOfertaForm()
...
def perfil_empresa(request, nombre_empresa):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid(): val = form.save(commit=False); val.empresa_nombre = nombre_empresa; val.save(); return redirect('perfil_empresa', nombre_empresa=nombre_empresa)
    else: form = ValoracionForm()
    val = Valoracion.objects.filter(empresa_nombre__icontains=nombre_empresa, aprobado=True).order_by('-fecha'); avg = val.aggregate(Avg('estrellas'))['estrellas__avg']
...
def postular_oferta(request, id):
    ...
    if Postulacion.objects.filter(oferta=oferta, candidato=candidato).exists(): messages.warning(request, "Ya has enviado tu perfil."); return redirect('detalle', id=id)
    Postulacion.objects.create(oferta=oferta, candidato=candidato)
    if oferta.usuario: Notificacion.objects.create(usuario=oferta.usuario, mensaje=f"📄 Nuevo candidato: {candidato.nombre} postuló a {oferta.titulo}", enlace=f"/gestion-oferta/{oferta.id}/candidatos/")
    messages.success(request, "¡Postulación enviada con éxito!"); return redirect('detalle', id=id)
...
def gestion_candidatos(request, id_oferta):
    ...
    if request.method == 'POST':
        post_id = request.POST.get('postulacion_id'); nuevo_estado = request.POST.get('nuevo_estado'); postulacion = get_object_or_404(Postulacion, id=post_id)
        postulacion.estado = nuevo_estado; postulacion.save()
        mensaje_candidato = ""
        ...
        if mensaje_candidato and postulacion.candidato.usuario: Notificacion.objects.create(usuario=postulacion.candidato.usuario, mensaje=mensaje_candidato, enlace=f"/oferta/{oferta.id}/")
        messages.success(request, "Estado actualizado."); return redirect('gestion_candidatos', id_oferta=id_oferta)
...
def exportar_candidatos_csv(request, id_oferta):
    ...
    response = HttpResponse(content_type='text/csv'); response['Content-Disposition'] = f'attachment; filename="Postulantes_{oferta.id}.csv"'; writer = csv.writer(response); writer.writerow(['Nombre', 'Email', 'Teléfono', 'Titulo', 'Estado', 'Fecha'])
    for post in oferta.postulaciones.all(): writer.writerow([post.candidato.nombre, post.candidato.email, post.candidato.telefono, post.candidato.titular, post.get_estado_display(), post.fecha.strftime("%d-%m-%Y")])
    return response

# --- CANDIDATOS ---

def lista_candidatos(request):
    candidatos = Candidato.objects.filter(publicado=True).order_by('-fecha_creacion'); rubro = request.GET.get('rubro'); region = request.GET.get('region')
...
def publicar_candidato(request):
    if request.method == 'POST':
        form = NuevoCandidatoForm(request.POST, request.FILES)
        if form.is_valid(): candidato = form.save(commit=False); candidato.usuario = request.user; candidato.save(); return redirect('pagina_exito')
    else: form = NuevoCandidatoForm()
    return render(request, 'publicar_candidato.html', {'form': form})

def descargar_cv_pdf(request, id):
    candidato = get_object_or_404(Candidato, id=id); template_path = 'pdf/cv_template.html'; context = {'candidato': candidato}
    response = HttpResponse(content_type='application/pdf'); response['Content-Disposition'] = f'attachment; filename="CV_{candidato.nombre}.pdf"'
    template = get_template(template_path); html = template.render(context); pisa_status = pisa.CreatePDF(html, dest=response)
...
def toggle_favorito(request, id_oferta):
    ...
    else: messages.success(request, "Guardado en favoritos ❤️")
...
def responder_pregunta(request, id_pregunta):
    ...
    if request.user == pregunta.oferta.usuario:
        ...
        if respuesta_texto:
            pregunta.respuesta = respuesta_texto; pregunta.fecha_respuesta = timezone.now(); pregunta.save()
...
def logout_usuario(request): logout(request); messages.info(request, "Sesión cerrada."); return redirect('home')
...
def panel_admin(request):
    total_ofertas = OfertaLaboral.objects.count(); total_candidatos = Candidato.objects.count(); total_usuarios = User.objects.count()
    ...
    publicadas = OfertaLaboral.objects.filter(publicada=True).count(); pendientes = OfertaLaboral.objects.filter(publicada=False).count()
...
def pagina_contacto(request):
    if request.method == 'POST': form = ContactoForm(request.POST); form.is_valid()
...
def mapa_empleos(request):
    ...
    for oferta in ofertas:
        ...
        if coords: marcadores.append({'lat': coords[0] + random.uniform(-0.02, 0.02), 'lng': coords[1] + random.uniform(-0.02, 0.02), 'titulo': oferta.titulo, 'empresa': oferta.empresa, 'url': f"/oferta/{oferta.id}/", 'tipo': oferta.get_tipo_display()})
